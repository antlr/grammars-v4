"""The script transforms the grammar to fit for the Antlr4ng target """
import sys
import re
import shutil
        ...
        with open(file_path, 'w', encoding="utf-8") as output_file:
            for line in input_file:
                line = re.sub(r"(\/\/ Insert here @header for lexer\.)",\
                    '@header {import { CLexerBase } from "./CLexerBase.js"}', line)
                line = re.sub(r"(\/\/ Insert here @header for parser\.)",\
                    '@header {import { CParserBase } from "./CParserBase.js"}', line)
