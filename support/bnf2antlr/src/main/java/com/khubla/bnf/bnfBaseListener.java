// Generated from com/khubla/bnf/bnf.g4 by ANTLR 4.2.2
package com.khubla.bnf;

import org.antlr.v4.runtime.ParserRuleContext;
import org.antlr.v4.runtime.misc.NotNull;
import org.antlr.v4.runtime.tree.ErrorNode;
import org.antlr.v4.runtime.tree.TerminalNode;

/**
 * This class provides an empty implementation of {@link bnfListener}, which can be extended to create a listener which only needs to handle a subset of the available methods.
 */
public class bnfBaseListener implements bnfListener {
   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterAlternative(@NotNull bnfParser.AlternativeContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterAlternatives(@NotNull bnfParser.AlternativesContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterElement(@NotNull bnfParser.ElementContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterEveryRule(@NotNull ParserRuleContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterId(@NotNull bnfParser.IdContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterLhs(@NotNull bnfParser.LhsContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterOneormore(@NotNull bnfParser.OneormoreContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterOptional(@NotNull bnfParser.OptionalContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterRhs(@NotNull bnfParser.RhsContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterRule_(@NotNull bnfParser.Rule_Context ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterRulelist(@NotNull bnfParser.RulelistContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterText(@NotNull bnfParser.TextContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void enterZeroormore(@NotNull bnfParser.ZeroormoreContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitAlternative(@NotNull bnfParser.AlternativeContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitAlternatives(@NotNull bnfParser.AlternativesContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitElement(@NotNull bnfParser.ElementContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitEveryRule(@NotNull ParserRuleContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitId(@NotNull bnfParser.IdContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitLhs(@NotNull bnfParser.LhsContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitOneormore(@NotNull bnfParser.OneormoreContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitOptional(@NotNull bnfParser.OptionalContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitRhs(@NotNull bnfParser.RhsContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitRule_(@NotNull bnfParser.Rule_Context ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitRulelist(@NotNull bnfParser.RulelistContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitText(@NotNull bnfParser.TextContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void exitZeroormore(@NotNull bnfParser.ZeroormoreContext ctx) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void visitErrorNode(@NotNull ErrorNode node) {
   }

   /**
    * {@inheritDoc}
    * <p>
    * The default implementation does nothing.
    * </p>
    */
   @Override
   public void visitTerminal(@NotNull TerminalNode node) {
   }
}