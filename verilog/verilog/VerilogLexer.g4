...
TEXT_MACRO_NAME : (LETTER | UNDERSCORE) (LETTER | UNDERSCORE | DECIMAL_DIGIT | DOLLAR_SIGN)* -> channel(DIRECTIVES), mode(MACRO_TEXT_MODE) ;
DEFINE_WHITE_SPACE : [ \t]+ -> channel(HIDDEN) ;

mode CONDITIONAL_DIRECTIVE_MODE;
CONDITIONAL_MACRO_NAME : (LETTER | UNDERSCORE) (LETTER | UNDERSCORE | DECIMAL_DIGIT | DOLLAR_SIGN)* -> channel(DIRECTIVES), mode(SOURCE_TEXT_MODE) ;
CONDITIONAL_WHITE_SPACE : [ \t]+ -> channel(HIDDEN) ;

mode MACRO_TEXT_MODE;
MACRO_TEXT : ~[\r\n\\]+ -> channel(DIRECTIVES) ;
MACRO_BACKSLASH_NEWLINE : BACKSLASH NEWLINE -> channel(DIRECTIVES) ;
MACRO_BACKSLASH_ESCAPE: BACKSLASH . -> channel(DIRECTIVES) ;
MACRO_ONE_LINE_COMMENT : DOUBLE_SLASH TEXT ~[\r\n]* -> channel(COMMENTS) ;
MACRO_BLOCK_COMMENT : SLASH_ASTERISK TEXT ASTERISK_SLASH -> channel(COMMENTS) ;
MACRO_NEWLINE : NEWLINE -> channel(HIDDEN), popMode ;

mode SOURCE_TEXT_MODE;
SOURCE_TEXT : ~[`\r\n\\]+ -> channel(DIRECTIVES) ;
SOURCE_GRAVE_ACCENT : '`' -> channel(DIRECTIVES), mode(DIRECTIVE_MODE) ;
SOURCE_BACKSLASH_ESCAPE: BACKSLASH . -> channel(DIRECTIVES) ;
SOURCE_ONE_LINE_COMMENT : DOUBLE_SLASH TEXT NEWLINE -> channel(COMMENTS) ;
SOURCE_BLOCK_COMMENT : SLASH_ASTERISK TEXT ASTERISK_SLASH -> channel(COMMENTS) ;
SOURCE_NEWLINE : NEWLINE -> channel(HIDDEN) ;
