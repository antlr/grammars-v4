...
mode DIRECTIVE_MODE;
// Compiler directives
DIRECTIVE_BEGIN_KEYWORDS : 'begin_keywords' SPACE_OR_TAB+ -> channel(DIRECTIVES), pushMode(BEGIN_KEYWORDS_DIRECTIVE_MODE) ;
DIRECTIVE_CELLDEFINE : 'celldefine' -> channel(DIRECTIVES), popMode ;
DIRECTIVE_DEFAULT_NETTYPE : 'default_nettype' SPACE_OR_TAB+ -> channel(DIRECTIVES), pushMode(DEFAULT_NETTYPE_DIRECTIVE_MODE) ;
DIRECTIVE_DEFINE : 'define' SPACE_OR_TAB+ -> channel(DIRECTIVES), pushMode(DEFINE_DIRECTIVE_MODE) ;
DIRECTIVE_ELSE : 'else' WHITE_SPACE+ -> channel(DIRECTIVES), pushMode(SOURCE_TEXT_MODE) ;
DIRECTIVE_ELSIF : 'elsif' SPACE_OR_TAB+ -> channel(DIRECTIVES), pushMode(CONDITIONAL_DIRECTIVE_MODE) ;
...
DIRECTIVE_IFDEF : 'ifdef' SPACE_OR_TAB+ -> channel(DIRECTIVES), pushMode(CONDITIONAL_DIRECTIVE_MODE) ;
DIRECTIVE_IFNDEF : 'ifndef' SPACE_OR_TAB+ -> channel(DIRECTIVES), pushMode(CONDITIONAL_DIRECTIVE_MODE) ;
DIRECTIVE_INCLUDE : 'include' SPACE_OR_TAB+ -> channel(DIRECTIVES), pushMode(INCLUDE_DIRECTIVE_MODE) ;
DIRECTIVE_LINE : 'line' SPACE_OR_TAB+ -> channel(DIRECTIVES), pushMode(LINE_DIRECTIVE_MODE) ;
DIRECTIVE_NOUNCONNECTED_DRIVE : 'nounconnected_drive' -> channel(DIRECTIVES), popMode ;
DIRECTIVE_PRAGMA : 'pragma' SPACE_OR_TAB+ -> channel(DIRECTIVES), pushMode(PRAGMA_MODE) ;
DIRECTIVE_RESETALL : 'resetall' -> channel(DIRECTIVES), popMode ;
DIRECTIVE_TIMESCALE : 'timescale' SPACE_OR_TAB+ -> channel(DIRECTIVES), pushMode(TIMESCALE_DIRECTIVE_MODE) ;
DIRECTIVE_UNCONNECTED_DRIVE : 'unconnected_drive' SPACE_OR_TAB+ -> channel(DIRECTIVES), pushMode(UNCONNECTED_DRIVE_DIRECTIVE_MODE) ;
DIRECTIVE_UNDEF : 'undef' SPACE_OR_TAB+ -> channel(DIRECTIVES), pushMode(UNDEF_DIRECTIVE_MODE) ;
DIRECTIVE_NEWLINE : NEWLINE -> channel(HIDDEN), popMode ;
TEXT_MACRO_USAGE : DIRECTIVE_IDENTIFIER SPACE_OR_TAB* (DIRECTIVE_LEFT_PARENTHESIS SPACE_OR_TAB* MACRO_LIST_OF_ACTUAL_ARGUMENTS SPACE_OR_TAB* DIRECTIVE_RIGHT_PARENTHESIS)? -> channel(DIRECTIVES), popMode ;
...
TEXT_MACRO_NAME : TEXT_MACRO_IDENTIFIER SPACE_OR_TAB* (DIRECTIVE_LEFT_PARENTHESIS SPACE_OR_TAB* MACRO_LIST_OF_FORMAL_ARGUMENTS SPACE_OR_TAB* DIRECTIVE_RIGHT_PARENTHESIS)? -> channel(DIRECTIVES), pushMode(MACRO_TEXT_MODE) ;
...
UNDEF_DIRECTIVE_IDENTIFIER : DIRECTIVE_IDENTIFIER -> channel(DIRECTIVES), popMode ;

mode CONDITIONAL_DIRECTIVE_MODE;
CONDITIONAL_DIRECTIVE_IDENTIFIER : DIRECTIVE_IDENTIFIER -> channel(DIRECTIVES), pushMode(SOURCE_TEXT_MODE) ;

mode INCLUDE_DIRECTIVE_MODE;
INCLUDE_FILENAME : DOUBLE_QUOTE ~["\r\n]+ DOUBLE_QUOTE -> channel(DIRECTIVES), popMode ;

mode LINE_DIRECTIVE_MODE;
LINE_NUMBER : NON_ZERO_UNSIGNED_NUMBER -> channel(DIRECTIVES), pushMode(LINE_FILENAME_LEVEL_MODE) ;
mode LINE_FILENAME_LEVEL_MODE;
LINE_FILENAME : DOUBLE_QUOTE ~["\r\n]+ DOUBLE_QUOTE -> channel(DIRECTIVES) ;
LINE_LEVEL : [0-2] -> channel(DIRECTIVES), popMode ;
LINE_WHITE_SPACE : SPACE_OR_TAB+ -> channel(HIDDEN) ;

mode TIMESCALE_DIRECTIVE_MODE;
TIMESCALE_ARGUMENTS : TIME_NUMBER SPACE_OR_TAB* TIME_UNIT SPACE_OR_TAB* TIMESCALE_SLASH SPACE_OR_TAB* TIME_NUMBER SPACE_OR_TAB* TIME_UNIT -> channel(DIRECTIVES), popMode ;
fragment TIME_NUMBER : ('1' | '10' | '100') -> channel(DIRECTIVES) ;
fragment TIME_UNIT : [mnpf]? 's' -> channel(DIRECTIVES) ;
fragment TIMESCALE_SLASH : '/' -> channel(DIRECTIVES) ;

mode UNCONNECTED_DRIVE_DIRECTIVE_MODE;
UNCONNECTED_DRIVE_VALUE : ('pull1' | 'pull0') -> channel(DIRECTIVES), popMode ;

mode PRAGMA_MODE;
PRAGMA_ARGUMENTS : PRAGMA_NAME SPACE_OR_TAB* (PRAGMA_EXPRESSION (SPACE_OR_TAB* DIRECTIVE_COMMA SPACE_OR_TAB* PRAGMA_EXPRESSION)*)? -> channel(DIRECTIVES), popMode ;
...
VERSION_SPECIFIER : DOUBLE_QUOTE ('1364-1995' | '1364-2001' | '1364-2001-noconfig' | '1364-2005') DOUBLE_QUOTE -> channel(DIRECTIVES), popMode ;
