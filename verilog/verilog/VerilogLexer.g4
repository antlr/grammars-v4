...
mode DIRECTIVE_MODE;
// Compiler directives
DIRECTIVE_BEGIN_KEYWORDS : 'begin_keywords' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(BEGIN_KEYWORDS_DIRECTIVE_MODE) ;
DIRECTIVE_CELLDEFINE : 'celldefine' -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;
DIRECTIVE_DEFAULT_NETTYPE : 'default_nettype' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_NETTYPE_DIRECTIVE_MODE) ;
DIRECTIVE_DEFINE : 'define' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(DEFINE_DIRECTIVE_MODE) ;
DIRECTIVE_ELSE : 'else' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(VERILOG_TEXT_MODE) ;
DIRECTIVE_ELSIF : 'elsif' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(CONDITIONAL_DIRECTIVE_MODE)  ;
DIRECTIVE_END_KEYWORDS : 'end_keywords' -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;
DIRECTIVE_ENDCELLDEFINE : 'endcelldefine' -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;
DIRECTIVE_ENDIF : 'endif' -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;
DIRECTIVE_IFDEF : 'ifdef' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(CONDITIONAL_DIRECTIVE_MODE) ;
DIRECTIVE_IFNDEF : 'ifndef' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(CONDITIONAL_DIRECTIVE_MODE) ;
DIRECTIVE_INCLUDE : 'include' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(INCLUDE_DIRECTIVE_MODE) ;
DIRECTIVE_LINE : 'line' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(LINE_DIRECTIVE_MODE) ;
DIRECTIVE_NOUNCONNECTED_DRIVE : 'nounconnected_drive' -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;
DIRECTIVE_PRAGMA : 'pragma' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(PRAGMA_MODE) ;
DIRECTIVE_RESETALL : 'resetall' -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;
DIRECTIVE_TIMESCALE : 'timescale' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(TIMESCALE_DIRECTIVE_MODE) ;
DIRECTIVE_UNCONNECTED_DRIVE : 'unconnected_drive' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(UNCONNECTED_DRIVE_DIRECTIVE_MODE) ;
DIRECTIVE_UNDEF : 'undef' [ \t]+ -> channel(DIRECTIVE_CHANNEL), mode(UNDEF_DIRECTIVE_MODE) ;
// Other symbols
DIRECTIVE_IDENTIFIER : (LETTER | UNDERSCORE) (LETTER | UNDERSCORE | DECIMAL_DIGIT | DOLLAR_SIGN)* -> channel(DIRECTIVE_CHANNEL) ;
DIRECTIVE_NEWLINE : NEWLINE -> channel(HIDDEN), mode(DEFAULT_MODE) ;
TEXT_MACRO_USAGE : [ \t]* '(' [ \t]* MACRO_LIST_OF_ACTUAL_ARGUMENTS [ \t]* ')' -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;
...
DEFAULT_NETTYPE_VALUE : ('wire' | 'tri' | 'tri0' | 'tri1' | 'wand' | 'triand' | 'wor' | 'trior' | 'trireg' | 'uwire' | 'none') -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;

mode DEFINE_DIRECTIVE_MODE;
TEXT_MACRO_NAME : TEXT_MACRO_IDENTIFIER [ \t]* ('(' [ \t]* MACRO_LIST_OF_FORMAL_ARGUMENTS [ \t]* ')')? -> channel(DIRECTIVE_CHANNEL), mode(MACRO_TEXT_MODE) ;
...
UNDEF_DIRECTIVE_IDENTIFIER : (LETTER | UNDERSCORE) (LETTER | UNDERSCORE | DECIMAL_DIGIT | DOLLAR_SIGN)* -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;

mode CONDITIONAL_DIRECTIVE_MODE;
CONDITIONAL_DIRECTIVE_IDENTIFIER : (LETTER | UNDERSCORE) (LETTER | UNDERSCORE | DECIMAL_DIGIT | DOLLAR_SIGN)* -> channel(DIRECTIVE_CHANNEL), mode(VERILOG_TEXT_MODE) ;

mode INCLUDE_DIRECTIVE_MODE;
FILENAME_STRING : '"' ~["\r\n]+ '"' -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;

mode LINE_DIRECTIVE_MODE;
LINE_ARGUMENTS : LINE_NUMBER [ \t]+ LINE_FILENAME [ \t]+ LINE_LEVEL -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;
fragment LINE_NUMBER : NON_ZERO_UNSIGNED_NUMBER ;
fragment LINE_FILENAME : '"' ~["\r\n]+ '"' ;
fragment LINE_LEVEL : [0-2] ;

mode TIMESCALE_DIRECTIVE_MODE;
TIMESCALE_ARGUMENTS : TIME_LITERAL [ \t]* '/' [ \t]* TIME_LITERAL -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;
...
UNCONNECTED_DRIVE_VALUE : ('pull1' | 'pull0') -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;

mode PRAGMA_MODE;
PRAGMA_ARGUMENTS : PRAGMA_NAME [ \t]* ('(' [ \t]* PRAGMA_EXPRESSION ([ \t]* ',' [ \t]* PRAGMA_EXPRESSION)* ')')? -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;
...
VERSION_SPECIFIER : '"' ('1364-1995' | '1364-2001' | '1364-2001-noconfig' | '1364-2005') '"' -> channel(DIRECTIVE_CHANNEL), mode(DEFAULT_MODE) ;

mode MACRO_TEXT_MODE;
MACRO_TEXT : ~[\r\n\\]+ -> channel(DIRECTIVE_CHANNEL) ;
MACRO_TEXT_ONE_LINE_COMMENT : DOUBLE_SLASH TEXT NEWLINE -> channel(HIDDEN) ;
MACRO_TEXT_BLOCK_COMMENT : SLASH_ASTERISK TEXT ASTERISK_SLASH -> channel(HIDDEN) ;
MACRO_TEXT_BACKSLASH_NEWLINE : BACKSLASH NEWLINE -> channel(DIRECTIVE_CHANNEL) ;
MACRO_TEXT_NEWLINE : NEWLINE -> channel(HIDDEN), mode(DEFAULT_MODE) ;

mode VERILOG_TEXT_MODE;
VERILOG_TEXT : ~[`\r\n\\]+ -> channel(DIRECTIVE_CHANNEL) ;
VERILOG_TEXT_ONE_LINE_COMMENT : DOUBLE_SLASH TEXT NEWLINE -> channel(HIDDEN) ;
VERILOG_TEXT_BLOCK_COMMENT : SLASH_ASTERISK TEXT ASTERISK_SLASH -> channel(HIDDEN) ;
VERILOG_TEXT_GRAVE_ACCENT : '`' -> channel(DIRECTIVE_CHANNEL), mode(DIRECTIVE_MODE) ;
