...
if ! command -v dotnet trash xml2 --version &> /dev/null
then
    echo "'dotnet' could not be found. Install Microsoft NET."
    exit 1
fi
...
do
    ...
    then
        ...
        then
            ...
            gtargets=`dotnet trash xml2 desc.xml | fgrep -e '/desc/targets' | awk -F '=' '{print $2}' | tr ';' '\n' | fgrep -e 'Java' | fgrep -v 'JavaScript'`
        fi
    ...
    do
        ...
        then
            ...
            then
                ...
                what=`dotnet trash xml2 desc.xml | grep inputs | head -1 | sed 's%^[^=]*=%%'`
            fi
            if [ "$what" == "" ]
            then
                dir=`pwd`
                ...
                then
                    ...
                    what=( `dotnet trash glob $p | grep -v '.errors$' | grep -v '.tree$'` )
                fi
            else
                dir=`pwd`
