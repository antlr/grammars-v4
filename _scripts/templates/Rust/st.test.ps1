...
# Individual parsing.
Get-Content "$filePath" | ForEach-Object { dotnet trash wdog ./target/release/<if(os_win)>test.exe<else>test<endif> -q -tee -tree $_ *>> parse.txt }
$status = $LASTEXITCODE
<else>
# Group parsing.
get-content "$filePath" | dotnet trash wdog ./target/release/<if(os_win)>test.exe<else>test<endif> -q -x -tee -tree *> parse.txt
$status = $LASTEXITCODE
<endif>
