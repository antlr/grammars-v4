...
for f in ${files[*]}
do
    dotnet trwdog ./target/release/test.exe/<if(os_win)>test.exe<else>test<endif> -q -tee -tree $f >> parse.txt 2>&1
    xxx="$?"
    if [ "$xxx" -ne 0 ]
    then
        status="$xxx"
...
<else>
# Group parsing.
echo "${files[*]}" | dotnet trwdog ./target/release/<if(os_win)>test.exe<else>test<endif> -q -x -tee -tree > parse.txt 2>&1
status=$?
<endif>
