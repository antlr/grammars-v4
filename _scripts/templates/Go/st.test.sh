...
for f in ${files[*]}
do
    dotnet trash wdog ./<if(os_win)>Test.exe<else>Test<endif> -q -tee -tree $f >> parse.txt 2>&1
    xxx="$?"
    if [ "$xxx" -ne 0 ]
    then
        status="$xxx"
...
<else>
# Group parsing.
echo "${files[*]}" | dotnet trash wdog ./<if(os_win)>Test.exe<else>Test<endif> -q -x -tee -tree > parse.txt 2>&1
status=$?
<endif>
