...
# Individual parsing.
Get-Content "$filePath" | ForEach-Object { dotnet trwdog ./<if(os_win)>Test.exe<else>Test<endif> -q -tee -tree $_ *>> parse.txt }
$status = $LASTEXITCODE
<else>
# Group parsing.
get-content "$filePath" | dotnet trwdog ./<if(os_win)>Test.exe<else>Test<endif> -q -x -tee -tree *> parse.txt
$status = $LASTEXITCODE
<endif>
