...
then
    ...
    do
        ...
        for ((i=1; i\<=n; i++))
        do
            dotnet trash wdog ./<if(os_win)>Test.exe<else>Test<endif> -prefix individual $f >> parse.txt 2>&1
            xxx="$?"
            if [ "$xxx" -ne 0 ]
            then
                status="$xxx"
...
if [[ "$type" == "group" ]]
then
    echo "${files[1]}" | dotnet trash wdog ./<if(os_win)>Test.exe<else>Test<endif> -x --perf --per-file -prefix group > /dev/null 2>&1
    for ((i=1; i\<=n; i++))
    do
        echo "${files[*]}" | dotnet trash wdog ./<if(os_win)>Test.exe<else>Test<endif> -x --perf --per-file -prefix group >> parse.txt 2>&1
        xxx="$?"
        if [ "$xxx" -ne 0 ]
        then
            status="$xxx"
