...
# Individual parsing.
Get-Content "$filePath" | ForEach-Object { dotnet trwdog dotnet run -q -tee -tree $_ *>> parse.txt }
$status = $LASTEXITCODE
<else>
# Group parsing.
get-content "$filePath" | dotnet trwdog dotnet run -q -x -tee -tree *> parse.txt
$status = $LASTEXITCODE
<endif>
