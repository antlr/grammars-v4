...
# Individual parsing.
Get-Content "$filePath" | ForEach-Object { dotnet trash wdog node Test.js -q -tee -tree $_ *>> parse.txt }
$status = $LASTEXITCODE
<else>
# Group parsing.
get-content "$filePath" | dotnet trash wdog node Test.js -q -x -tee -tree *> parse.txt
$status = $LASTEXITCODE
<endif>
