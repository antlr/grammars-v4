...
# Individual parsing.
Get-Content "tests.txt" | ForEach-Object { trwdog ./build/<if(os_win)>Release/<endif><exec_name> -q -tee -tree $_ *>> parse.txt }
<else>
# Group parsing.
get-content "tests.txt" | trwdog ./build/<if(os_win)>Release/<endif><exec_name> -q -x -tee -tree *> parse.txt
$status = $LASTEXITCODE
<endif>
