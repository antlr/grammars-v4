...
for f in ${files[*]}
do
    trwdog ./build/<if(os_win)>Release/<endif><exec_name> -prefix individual $f >> parse.txt 2>&1
    xxx="$?"
    if [ "$xxx" -ne 0 ]
    then
        status="$xxx"
...
# Group parsing.
echo "${files[*]}" | trwdog ./build/<if(os_win)>Release/<endif><exec_name> -x -prefix group >> parse.txt 2>&1
status=$?
