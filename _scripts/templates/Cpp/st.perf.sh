...
if [[ "$type" == "group" ]]
then
    echo "${files[1]}" | dotnet trash wdog ./build/<if(os_win)>Release/<endif><exec_name> -x -prefix group > /dev/null 2>&1
    for ((i=1; i\<=n; i++))
    do
        echo "${files[*]}" | dotnet trash wdog ./build/<if(os_win)>Release/<endif><exec_name> -x -prefix group >> parse.txt 2>&1
        xxx="$?"
        if [ "$xxx" -ne 0 ]
        then
            status="$xxx"
