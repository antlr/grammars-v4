...
for f in ${files[*]}
do
    dotnet trash wdog ./build/<if(os_win)>Release/<endif><exec_name> -q -tee -tree $f >> parse.txt
    xxx="$?"
    if [ "$xxx" -ne 0 ]
    then
        status="$xxx"
...
<else>
# Group parsing.
echo "${files[*]}" | dotnet trash wdog ./build/<if(os_win)>Release/<endif><exec_name> -q -x -tee -tree > parse.txt 2>&1
status="$?"
<endif>
