...
# Individual parsing.
Get-Content "tests.txt" | ForEach-Object { trwdog pwsh -command "ts-node Test.js -q -tee -tree $_" *>> parse.txt }
<else>
# Group parsing.
get-content "tests.txt" | trwdog pwsh -command "ts-node Test.js -q -x -tee -tree" *> parse.txt
$status=$LASTEXITCODE
<endif>
