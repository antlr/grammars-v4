...
then
    ...
    do
        ...
        for ((i=1; i\<=n; i++))
        do
            dotnet trash wdog sh -c "exec npx tsx Test.js -prefix individual $f" >> parse.txt 2>&1
            xxx="$?"
            if [ "$xxx" -ne 0 ]
            then
                status="$xxx"
...
if [[ "$type" == "group" ]]
then
    echo "${files[1]}" | dotnet trash wdog sh -c "exec npx tsx Test.js -x --perf --per-file -prefix group" > /dev/null 2>&1
    for ((i=1; i\<=n; i++))
    do
        echo "${files[*]}" | dotnet trash wdog sh -c "exec npx tsx Test.js -x --perf --per-file -prefix group" >> parse.txt 2>&1
        xxx="$?"
        if [ "$xxx" -ne 0 ]
        then
            status="$xxx"
