...
then
    ...
    do
        ...
        for ((i=1; i\<=n; i++))
        do
            dotnet trash wdog php -d memory_limit=1G Test.php -prefix individual $f >> parse.txt 2>&1
            xxx="$?"
            if [ "$xxx" -ne 0 ]
            then
                status="$xxx"
...
if [[ "$type" == "group" ]]
then
    echo "${files[1]}" | dotnet trash wdog php -d memory_limit=1G Test.php -x --perf --per-file -prefix group > /dev/null 2>&1
    for ((i=1; i\<=n; i++))
    do
        echo "${files[*]}" | dotnet trash wdog php -d memory_limit=1G Test.php -x --perf --per-file -prefix group >> parse.txt 2>&1
        xxx="$?"
        if [ "$xxx" -ne 0 ]
        then
            status="$xxx"
