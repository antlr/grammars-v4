...
# Individual parsing.
Get-Content "$filePath" | ForEach-Object { dotnet trash wdog php -d memory_limit=1G Test.php -q -tee -tree $_ *>> parse.txt }
$status = $LASTEXITCODE
<else>
# Group parsing.
get-content "$filePath" | dotnet trash wdog php -d memory_limit=1G Test.php -q -x -tee -tree *> parse.txt
$status = $LASTEXITCODE
<endif>
