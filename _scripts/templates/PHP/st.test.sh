...
for f in ${files[*]}
do
    dotnet trash wdog php -d memory_limit=1G Test.php -q -tee -tree $f >> parse.txt 2>&1
    xxx="$?"
    if [ "$xxx" -ne 0 ]
    then
        status="$xxx"
...
<else>
# Group parsing.
echo "${files[*]}" | dotnet trash wdog php -d memory_limit=1G Test.php -q -x -tee -tree > parse.txt 2>&1
status=$?
<endif>
