...
public abstract class GDScriptLexerBase : Lexer {
	...
	public GDScriptLexerBase(ICharStream input)
			: base(input)
	{
	}
	public GDScriptLexerBase(ICharStream input, TextWriter output, TextWriter errorOutput)
	...
	{
		var dedent = CommonToken(GDScriptParser.DEDENT, "");
	...
	public override IToken NextToken()
	{
		...
		if (((ICharStream)InputStream).LA(1) == Eof && Indents.Count != 0)
		{
			...
			this.Emit(CommonToken(GDScriptParser.NEWLINE, "\n"));
			...
			Emit(CommonToken(GDScriptParser.Eof, "<EOF>"));
		}
	...
	public void onNewLine(){
		...
		else
		{
			if (LastToken != null)
                        {
				Emit(CommonToken(GDScriptParser.NEWLINE, newLine));
			}
			...
			if (indent == previous)
			{
				...
				Indents.Push(indent);
				Emit(CommonToken(GDScriptParser.INDENT, spaces));
			}
			else {
