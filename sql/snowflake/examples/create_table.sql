...
CREATE HYBRID TABLE ht2 (c1 INT PRIMARY KEY, c2 VARCHAR(10),INDEX idx_c2 (c2));
--CREATE HYBRID TABLE mytable (customer_id INT AUTOINCREMENT PRIMARY KEY, email VARCHAR(255) UNIQUE,  extended_customer_info VARIANT,  INDEX index_full_name (full_name));
CREATE OR REPLACE HYBRID TABLE ht1pk (COL1 NUMBER(38,0) NOT NULL COMMENT 'Primary key', CONSTRAINT PKEY_1 PRIMARY KEY (COL1));
CREATE OR REPLACE HYBRID TABLE TEST_HTABLE COMMENT = 'TEST' (C INT PRIMARY KEY) WITH ROW ACCESS POLICY RLS ON (C) WITH TAG (CONF='C2');
CREATE OR REPLACE HYBRID TABLE TEST_HTABLE (C INT PRIMARY KEY) COMMENT = 'TEST'  WITH TAG (CONF='C2') WITH ROW ACCESS POLICY RLS ON (C);
