 ...
 * Copyright (c) 2015 Ivan Kochurkin (KvanTTT)
 * Translated from https://github.com/porcelli/plsql-parser
 * Licence: MIT.
 */
...
partition_by_clause
    :    '('
            ...
            (    ANY 
            |    (HASH|RANGE) '(' column_name (',' column_name)* ')'
            )
            streaming_clause?
        ')'
    ;
...
package_body
    :    BODY package_name (IS | AS)
        package_obj_body*
        (BEGIN seq_of_statements|END package_name?)
    ;

package_spec
    :    package_name invoker_rights_clause? (IS | AS)
        package_obj_spec*
        END package_name?
    ;
...
non_dml_trigger
    :    (BEFORE|AFTER) non_dml_event (OR non_dml_event)*
...
dml_event_clause
    :    dml_event_element (OR dml_event_element)*
        ON 
...
type_body
    :    BODY type_name
        (IS|AS) type_body_elements (',' type_body_elements)*
        END
    ;
...
proc_decl_in_type
    :    PROCEDURE procedure_name
        ...
        (IS|AS) 
            (call_spec|DECLARE? declare_spec* body ';')
    ;
...
statement
    :    CREATE swallow_to_semi (';'|EOF)
    |    ALTER swallow_to_semi  (';'|EOF)
    |    GRANT swallow_to_semi  (';'|EOF)
    |    TRUNCATE swallow_to_semi  (';'|EOF)
    |    body
    |    block
...
loop_statement
    :    label_name?
        (WHILE condition | FOR cursor_loop_param)?
        LOOP
        seq_of_statements
        END LOOP label_name?
    ;
...
unpivot_in_clause
    :    IN
        '('
            unpivot_in_elements (',' unpivot_in_elements)*
        ')'
    ;
...
grouping_sets_clause
    :    GROUPING SETS 
        '(' grouping_sets_elements (',' grouping_sets_elements)* ')'
    ;
...
reference_model
    :    REFERENCE reference_model_name ON 
            '(' subquery ')' model_column_clauses 
            cell_reference_options*
    ;
...
model_column_clauses
    :    model_column_partition_part?
        DIMENSION BY model_column_list MEASURES model_column_list
    ;
...
merge_insert_clause
    :    WHEN NOT MATCHED THEN INSERT 
        ('(' column_name (',' column_name)* ')')?
        VALUES expression_list where_clause?
    ;
...
lock_table_statement
    :    LOCK TABLE 
        lock_table_element (',' lock_table_element)* 
        IN lock_mode MODE wait_nowait_part?
    ;
...
static_returning_clause
    :    (RETURNING|RETURN) expression (',' expression)* 
        into_clause
    ;
...
multiset_expression
    ...
    (    multiset_type OF? concatenation)?
    ;
...
datetime_expression
    :    model_expression
    (    AT (LOCAL|TIME ZONE concatenation_wrapper)
    |    interval_expression
    )?
    ;
...
single_column_for_loop
    :    FOR column_name 
    (    IN expression_list
    |    for_like_part? FROM ex1=expression
            TO ex2=expression for_increment_decrement_type ex3=expression     
    )
    ;
...
multi_column_for_loop
    :    FOR '(' column_name (',' column_name)* ')' IN
        '('
            (    subquery
...
standard_function
    ...
    |  COUNT
            '('
                ( '*' | (DISTINCT|UNIQUE|ALL)? concatenation_wrapper )
            ')' over_clause?
    |    (CAST|XMLCAST) 
            '('
                ( MULTISET '(' subquery ')'
    ...
    |    CHR
            '(' 
                concatenation_wrapper USING NCHAR_CS 
            ')'
    |    COLLECT
            '(' 
                (DISTINCT|UNIQUE)? concatenation_wrapper collect_order_by_part?
            ')'
    |    within_or_over_clause_keyword 
            function_argument within_or_over_part+
    |    DECOMPOSE
            '(' 
                concatenation_wrapper (CANONICAL|COMPATIBILITY)? 
            ')'
    |    EXTRACT
            '('
                regular_id FROM concatenation_wrapper 
            ')'
    |    (FIRST_VALUE|LAST_VALUE) function_argument_analytic
    ...
    |    TRANSLATE
            '(' 
                expression_wrapper (USING (CHAR_CS|NCHAR_CS))? 
                    (',' expression_wrapper)* 
            ')'
    |    TREAT
            '('
                expression_wrapper AS REF? type_spec 
            ')'
    |    TRIM
            '(' 
                ((LEADING|TRAILING|BOTH)? quoted_string? FROM)?
                concatenation_wrapper
            ')'
    |    XMLAGG
            '(' 
    ...
    |    XMLPARSE
            '(' 
                (DOCUMENT|CONTENT) concatenation_wrapper WELLFORMED?
            ')'
            ('.' general_element_part)?
    |    XMLPI
            '(' 
                (    NAME id
                |    EVALNAME concatenation_wrapper
                )
                (',' concatenation_wrapper)?
            ')'
            ('.' general_element_part)?
    ...
    |    XMLSERIALIZE
            '('
                (DOCUMENT|CONTENT)
                concatenation_wrapper (AS type_spec)?
...
cost_matrix_clause
    :    COST
    (    MODEL AUTO?
    |    '(' cost_class_name (',' cost_class_name)* ')' VALUES 
            expression_list
    )
    ;
...
xml_passing_clause
    :    PASSING (BY VALUE)?
            expression_wrapper column_alias? (',' expression_wrapper column_alias?)
    ;
...
xml_table_column
    :    xml_column_name
    (    FOR ORDINALITY
    |    type_spec (PATH concatenation_wrapper)? xml_general_default_part?
    )
    ;
...
whenever_command
    :    WHENEVER (SQLERROR|OSERROR) (EXIT (SUCCESS|FAILURE|WARNING) (COMMIT|ROLLBACK) | CONTINUE (COMMIT|ROLLBACK|NONE))
    ;
...
constant
    ...
    |    INTERVAL (quoted_string | bind_variable | general_element_part)
         ...
         ( TO
             ( DAY | HOUR | MINUTE | SECOND ('(' (UNSIGNED_INTEGER | bind_variable) ')')? )
         )?
    |    numeric
    |    DATE quoted_string
    |    quoted_string
