...
do
    ...
    echo yo `pwd`
    dotnet trash gen "${trgen_opts[@]}"

    d=`echo Generated-$target`
    if [ ! -d $d ]
    then
	popd
	continue
    ...
    printf '%s\n' "${files[@]}" | bash run.sh -x > /dev/null 2>&1
    ...
    for ((run=1; run<=N; run++)); do
        echo "=== $testname,$target run $run/$N ==="
        runfile="$tmpdir/$(echo "$testname" | tr '/' '_')_run${run}.txt"
        printf '%s\n' "${files[@]}" | bash run.sh -x 2>&1 | tee "$runfile"
    ...
    table_rows+=("| ${testname},${target} | ${pt_stat} | ${ot_stat} | ${tt_stat} | ${tps_stat} | ${warm_tps_stat} | ${speedup_stat} |")
    popd > /dev/null
done
