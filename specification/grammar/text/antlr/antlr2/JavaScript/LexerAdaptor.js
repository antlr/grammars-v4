...
import ANTLRv2Lexer from './ANTLRv2Lexer.js';
...
export default class LexerAdaptor extends antlr4.Lexer
{
    ...
    {
        if (this.inLexerRule()) {
            this.pushMode(ANTLRv2Lexer.LexerCharSet);
            this.more();
        } else {
            this.pushMode(ANTLRv2Lexer.Argument);
        }
    }
    ...
    handleEndArgument()
    {
        ...
        if (this._modeStack.length > 0) {
            this._type = ANTLRv2Lexer.ARGUMENT_CONTENT;
        }
    }
    ...
    handleEndAction()
    {
        ...
        if (isActionWithinAction) {
            this._type = ANTLRv2Lexer.ACTION_CONTENT;
        }
    }

    handleOptionsLBrace() {
		this._type = ANTLRv2Lexer.LBRACE;
    }

    emit()
    {
        if ((this._type == ANTLRv2Lexer.OPTIONS || this._type == ANTLRv2Lexer.TOKENS)
        ...
        } else if (this._type == ANTLRv2Lexer.RBRACE && this.getCurrentRuleType() == this.PREQUEL_CONSTRUCT) { // exit prequel construct
            this.setCurrentRuleType(antlr4.Token.INVALID_TYPE);
        } else if (this._type == ANTLRv2Lexer.AT && this.getCurrentRuleType() == antlr4.Token.INVALID_TYPE) { // enter action
            this.setCurrentRuleType(ANTLRv2Lexer.AT);
        } else if (this._type == ANTLRv2Lexer.END_ACTION && this.getCurrentRuleType() == ANTLRv2Lexer.AT) { // exit action
            this.setCurrentRuleType(antlr4.Token.INVALID_TYPE);
        } else if (this._type == ANTLRv2Lexer.ID) {
            var firstChar = this._input.getText(this._tokenStartCharIndex, this._tokenStartCharIndex);
            var c = firstChar.charAt(0);
            if (c == c.toUpperCase()) {
                this._type = ANTLRv2Lexer.TOKEN_REF;
            } else {
                this._type = ANTLRv2Lexer.RULE_REF;
            }
        ...
        } else if (this._type == ANTLRv2Lexer.SEMI) { // exit rule def
            this.setCurrentRuleType(antlr4.Token.INVALID_TYPE);
        }
    ...
    inLexerRule() {
        return this.getCurrentRuleType() == ANTLRv2Lexer.TOKEN_REF;
    }

    inParserRule() { // not used, but added for clarity
        return this.getCurrentRuleType() == ANTLRv2Lexer.RULE_REF;
    }
