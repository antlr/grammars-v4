#!/usr/bin/env bash
# test-dotnet.sh — Compile and run C# 8 example files using the installed .NET SDK.
#
# Usage:  bash test-dotnet.sh [examples-dir]
#
# Each CSharp8*.cs file in examples-dir is compiled into its own temporary project
# with LangVersion=8.0 and then executed.  The script reports PASS/FAIL per file
...
for cs_file in "$EXAMPLES_DIR"/CSharp8*.cs; do
    [ -f "$cs_file" ] || continue
