#!/usr/bin/env bash
# test.sh — Clone Roslyn at the C# 8 tag, extract C# source snippets from
...
for path in files:
    ...
    for m in VERBATIM.finditer(text):
        raw = m.group(1)
        # Decode verbatim-string escapes: "" -> "
        # Also decode format-template brace escapes: {{ -> { and }} -> }
        # (test strings used as String.Format templates encode literal braces this way)
