...
Fix: clean out zero-length `.errors` files from the test directory (check `pom.xml` for
...
**Caution:** `git clean -f` will also remove any untracked test files you added but haven't
staged with `git add`. If you've added new test inputs without staging them, manually delete
only the zero-length `.errors` files instead.
