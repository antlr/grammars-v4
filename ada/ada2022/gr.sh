...
targets=( Antlr4ng Cpp CSharp Dart Go Java JavaScript Python3 TypeScript )
c=${#targets[@]}
sq=`seq -s " " 1 $c | tr -d '\n'`
n=`grep SampleSize= Generated-${targets[0]}-1/parse.txt | sed 's/^[^=]*=//'`
name=`head -2 Generated-${targets[0]}-1/parse.txt | tail -1 | tr -d '\n' | tr -d '\r'`

# collect grouped data in one file. Order important.
for t in ${targets[@]}
do
    echo $t
    grep "group Total Time" Generated-$t-1/parse.txt >> times.txt
done
