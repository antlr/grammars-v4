...
--      pragma Restrictions (No_Task_Allocators) is included.
--
-- TEST DESCRIPTION:
--      Pragma Restrictions No_Task_Allocators is called out in a separate
--      text file.  A library level package with code that uses task 
--      allocators is contained in another file.  A main unit which "withs" 
--      the package is declared in a third file.  These three files are 
--      brought together in a single partition.  The method by which the
...
--      made to include code containing  task allocators when the partition
--      has a Restriction of No_Task_Allocators
--
...
with LXD70051;   -- include the task allocators
with Report;
         
procedure LXD70052 is


begin -- LXD70052

   Report.Test ("LXD70052", "Tasking Restrictions: No_Task_Allocators");

   Report.Failed ("Should not execute");

   -- abort the tasks so the test does not hang
   LXD70051.Close_Control.Disaster_Shut_Down;  
