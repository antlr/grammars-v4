...
with Ada.Assertions;
with Ada.Tags;
with C611A040.Child.Grandchild;
with F611A00;
with Report;

procedure C611A043 is
   ...
   My_Object : C611A040.Child.Object;

   My_Triangle : C611A040.Child.Grandchild.Triangle :=
                  C611A040.Child.Grandchild.Make_Triangle
                   (A => 3.0, B => 4.0, C => 5.0);

   procedure Test_Get_Distance (O    : in C611A040.Child.Object'Class;
                                Code : in Character) is
      -- Test a dispatching call of Distance.

      use type Ada.Tags.Tag;

      function TC_Eval_Object (O    : in C611A040.Child.Object'Class)
         return C611A040.Child.Object'Class is
      ...
      begin
         ...
         if F611A00.TC_Point_Distance_Pre_Class_Called or
      ...
      function Call_Get_Distance (O : in C611A040.Child.Object'Class)
         return Float is
         Inner_Distance : Float;
      begin
         -- Dispatching call
         Inner_Distance := C611A040.Child.Distance (TC_Eval_Object (O));
         ...
         if not F611A00.TC_Point_Distance_Post_Class_Called then
            Report.Failed
   ...
   begin
      ...
      begin
         ...
         if O'Tag = C611A040.Child.Grandchild.Triangle'Tag then
            -- Check the following objective:
            --    For an interface type T and a primitive subprogram S
            --    of T that has a class-wide postcondition expression E,
            ...
            --    a concrete type derived from T.
            ...
            if not (F611A00.TC_Point_Distance_Post_Class_Called and
                    F611A00.TC_Triangle_Distance_Post_Class_Called) then
               Report.Failed
            ...
            -- Check the following objective:
            --    For an interface type T and a primitive subprogram S
            --    of T that has a class-wide precondition expression E,
            --    check that for a dynamically tagged dispatching call of S,
            --    calls to primitive operations of T within E invoke the
            --    bodies appropriate for the controlling tag, even if it is not
            --    a concrete type derived from T.
            ...
            -- routines are called.
            if (F611A00.TC_Point_Distance_Pre_Class_Called and then
         ...
         else -- O'Tag = C611A040.Child.Object'Tag
            -- Check the following objective:
            --    For an interface type T and a primitive subprogram S
            --    of T that has a class-wide postcondition expression E,
            --    check that for a dynamically tagged dispatching call of S,
            --    calls to primitive operations of T within E invoke the
            --    bodies appropriate for the controlling tag, even if it is not
            --    a concrete type derived from T. [In this case, it is a
            --    concrete type derived from T, but it's still worth testing.]
            ...
            if not (F611A00.TC_Object_Distance_Called and
                    F611A00.TC_Point_Distance_Post_Class_Called) then
               Report.Failed
                  ("Hasn't called the classwide postcondition of the " &
                   "body dispatched to - " & Code);
               F611A00.TC_Output;
   ...
   procedure Test_Get_Distance2 (O    : in C611A040.Child.Object;
                                 Code : in Character) is
      ...
      function Call_Get_Distance (O : in C611A040.Child.Object)
         return Float is
         Inner_Distance : Float;
      begin
         ...
         Inner_Distance :=
            C611A040.Child.Distance (
               C611A040.Child.Object(TC_Eval_Object (O)));
   ...
   begin
      ...
      begin
         ...
         --    For a concrete type C derived from an interface type T and a
         --    primitive subprogram S of T that has a class-wide
         --    precondition expression E, check that for a call of S that is
         --    statically bound to type C, calls to primitive operations of T
         --    within E invoke the bodies appropriate for T, even if the tag of
         --    the controlling parameter object is not C.
   ...
   procedure Test_Get_Distance3 (O    : in C611A040.Child.Grandchild.Triangle;
                                 Code : in Character) is
      -- Test a statically bound call of Distance (where the real tag
      -- might differ from the statically determined tag).
      ...
      function Call_Get_Distance (O : in C611A040.Child.Grandchild.Triangle)
         return Float is
         Inner_Distance : Float;
      begin
         -- Statically bound call:
         --Inner_Distance := O.Distance;
         Inner_Distance :=
            C611A040.Child.Grandchild.Triangle(TC_Eval_Object (O)).Distance;
...
begin
   ...
   -- As the denoted program C611A040.Child.Distance is a primitive of Object,
   -- only its Pre'Class should be checked, not the class-wide precondition of
   -- the body dispatched to (C611A040.Child.Grandchild.Distance, whose
   -- class-wide precondition is an 'or' of C611A040.Distance's and
   -- C611A040.Child.Grandchild.Distance's Pre'Classes).
   ...
   Test_Get_Distance2 (C611A040.Child.Object(My_Triangle), Code => 'A');
   ...
   Test_Get_Distance2 (C611A040.Child.Object(My_Triangle), Code => 'B');
