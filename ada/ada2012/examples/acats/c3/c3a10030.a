...
--     specifications, and that a client package need not with a package
--     containing a type that it is not interested in, even if that type
--     only has a limited view.
--
--     We use the classic Employee and Department example as outlined in
--     AI95-00326-1. In this example, we have a third package containing
--     employee operations (named C3A10030 here). These operations only
--     use entities declared in package Employees (named C3A1003E here),
--     but this package uses the limited view of a type imported from
--     package Departments (named C3A1003E here). We check that calls
...
--            C3A10030 to the compilation environment. (Careful: the
--            compilation of the normal view of the file C3A10030 should
--            not be done at this time.)
--         2) Do the steps required to add the limited view of the file
--            C3A10031 to the environment.
--         3) Compile the file C3A10030 (and add the results to the
--            environment).
--         4) Compile the file C3A10031 (and add the results to the
--            environment).
--         5) Compile the file C3A10032 (and add the results to the
--            environment).
...
--
--!
limited with C3A1003E;
package C3A1003D is -- Departments
   type Emp_Ptr is access all C3A1003E.Employee'Class;
