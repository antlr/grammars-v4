...
package F360B00 is
    ...
    type Float_Array is array (Index) of Small_Float
        with Default_Component_Value => Flt_Value;

    Fix_Value : constant := 3.75;

    type Small_Fixed is delta 0.125 range -4.0 .. 4.0
        with Small => 0.125;
    type Fixed_Array is array (Index) of Small_Fixed
        with Default_Component_Value => Fix_Value;

    Dec_Value : constant := -1.00;

    type Small_Dec is delta 0.01 digits 4; -- Range -10.00 .. 10.00
    type Dec_Array is array (Index) of Small_Dec
        with Default_Component_Value => Dec_Value;
