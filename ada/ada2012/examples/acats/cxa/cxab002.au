...
with Ada.Strings.Wide_Bounded;           use Ada.Strings.Wide_Bounded;
with Ada.Wide_Text_IO;
with Ada.Wide_Text_IO.Wide_Bounded_IO;

with Report; use Report;

procedure CXAB002 is
begin
   Test ("CXAB002", "Check file operations defined in Wide_Text_IO.Wide_Bounded_IO");

   declare
      package BW_Str is new
        Ada.Strings.Wide_Bounded.Generic_Bounded_Length (5);
      package BW_Str_IO is new
        Ada.Wide_Text_IO.Wide_Bounded_IO (BW_Str);

      use Ada.Wide_Text_IO;
      use BW_Str;
      use BW_Str_IO;

      B : Bounded_Wide_String := To_Bounded_Wide_String ("junk");
      F : File_Type;
      S1 : Bounded_Wide_String := To_Bounded_Wide_String ("EFGHI");
      S2 : Bounded_Wide_String := To_Bounded_Wide_String ("πr²h");
      Long : Wide_String := "Really Long";
   begin
      --  Put (File_Type; Bounded_Wide_String
      ...
      --  Put_Line (File_Type; Bounded_Wide_String)
      ...
      --  Get_Line (File_Type) return Bounded_Wide_String);
      ...
      if B /= S1 then
         Failed ("Get_Line (File_Type) return Bounded_Wide_String");
      end if;

      --  Get_Line (File_Type; out Bounded_Wide_String)

      Open     (F, In_File, Report.Legal_File_Name(X => 2));
      Get_Line (F, B);

      if B /= S1 then
         Failed ("Get_Line (File_Type; out Bounded_Wide_String) - S1");
      end if;

      Get_Line (F, B);

      if B /= S2 then
         Failed ("Get_Line (File_Type; out Bounded_Wide_String) - S2");
      end if;
      ...
      begin
         ...
         if B /= S2 then -- Stop dead code elimination.
            Failed ("Get_Line (File_Type; out Bounded_Wide_String) - Long");
         end if;
      exception
