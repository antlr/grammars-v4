...
--      22 Dec 2014 RLB  Created Wide_Wide_Unbounded_IO version from Bounded_IO version.
--!
...
procedure CXAB005 is
begin
   Test ("CXAB005", "Check file operations defined in Wide_Wide_Text_IO.Wide_Wide_Unbounded_IO");

   declare
      use Ada.Wide_Wide_Text_IO;

      B : Unbounded_Wide_Wide_String := To_Unbounded_Wide_Wide_String ("junk");
      F : File_Type;
      S1 : Unbounded_Wide_Wide_String := To_Unbounded_Wide_Wide_String ("Ice Age");
      S2 : Unbounded_Wide_Wide_String := To_Unbounded_Wide_Wide_String (" Trail");
      S3 : Unbounded_Wide_Wide_String := To_Unbounded_Wide_Wide_String ("A rather long string " &
        "in which we write Πλάτων and Чайковский because we can!");
   begin
      --  Put (File_Type; Unbounded_Wide_Wide_String
      ...
      --  Put_Line (File_Type; Unbounded_Wide_Wide_String)
      ...
      --  Get_Line (File_Type) return Unbounded_Wide_Wide_String);
      ...
      if B /= S1 then
         Failed ("Get_Line (File_Type) return Unbounded_Wide_Wide_String");
      end if;

      --  Get_Line (File_Type; out Unbounded_Wide_Wide_String)

      Open     (F, In_File, Report.Legal_File_Name(X => 2));
      Get_Line (F, B);

      if B /= (S1 & S2) then
         Failed ("Get_Line (File_Type; out Unbounded_Wide_Wide_String) - S1 + S2");
      end if;

      Get_Line (F, B);

      if B /= S3 then
         Failed ("Get_Line (File_Type; out Unbounded_Wide_Wide_String) - S3");
      end if;
