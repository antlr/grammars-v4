...
--      of stream elements specified by their Stream_Size attribute. Part 1:
--      attribute definition clauses, confirming values.
--
-- TEST DESCRIPTION:
...
procedure CDD2B01 is
   ...
   for Small_Int'Stream_Size use Basic_Int'Stream_Size;  -- ANX-C RQMT. {4;1}

   type Big_Stream is range -9999 .. 9999;
   for Big_Stream'Stream_Size use Larger_Int'Stream_Size;-- ANX-C RQMT. {4;1}

   type Small_Mod is mod 2 ** 8;
   for Small_Mod'Stream_Size use Basic_Mod'Stream_Size;  -- ANX-C RQMT. {4;1}

   type Quarter_Fix is delta 0.25 range -4.0 .. 4.0;
   for Quarter_Fix'Small use 0.25;
   for Quarter_Fix'Stream_Size use Basic_Fix'Stream_Size;-- ANX-C RQMT. {4;1}

   type Small_Flt is digits 3;
   for Small_Flt'Stream_Size use Basic_Flt'Stream_Size;  -- ANX-C RQMT. {4;1}
...
begin
   Report.Test ("CDD2B01", "Check that the default stream attributes read " &
                           "and write the specified number of stream " &
                           "elements. Part 1: attribute definition clauses " &
                           "with confirming stream sizes");
