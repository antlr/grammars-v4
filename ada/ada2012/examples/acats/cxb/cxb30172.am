...
--      CXB30170 rather than directly using C language main subprogram.
--
--      CXB30170 is imported using pragma Import. It will call an Ada procedure
--      in package CXB30171 named Ada_Doubler, which is exported using
--      pragma Export. This routine doubles all of it's parameters.
--      The C function then takes the result of the first parameter and
--      returns it adding the value of Ada object CXB30171.Global. All
...
--      The files CXB30170.C must be compiled with a C
...
--
--      Note that the C code included in CXB30170.C conforms
...
with CXB30171;
procedure CXB30172 is
begin

   Report.Test ("CXB3017", "Check that exported Ada subprogram and object " &
                           "can be used from a C function. Part 1: " &
                           "Using pragma Export");
   ...
   declare
     ...
     pragma Import (Convention    => C,                       -- N/A => ERROR
                    Entity        => Do_It,
                    External_Name => Impdef.CXB30170_External_Name);

   begin

      -- Change the value of the global:
      CXB30171.Global := Interfaces.C.Int(Report.Ident_Int(10));

      -- Check that the imported version of C function CXB30170 produces
      -- the correct results.
